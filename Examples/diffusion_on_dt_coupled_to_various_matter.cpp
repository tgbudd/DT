...
int main(int argc, char* argv[])
{
	...
	int measurewhat = param.Read<int>("measure diffusion (1=yes,0=no)");
	...
	{	
		std::vector<boost::array<bool,3> > intree(n);
		builder.getSpanningTree(intree);
		SpanningTree * spanningtree = new SpanningTree( &triangulation );
		spanningtree->Initialize(intree);
		dom = spanningtree;
	...
	diffusion.setMeasureDistanceOnly( measurewhat==0 );
